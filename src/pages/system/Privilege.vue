<template>
  <div class="product">
    ...
    >
      ...
      <el-table-column label="权限名" prop="name" width="300"></el-table-column>
      <el-table-column label="权限类型" prop="type" width="300"></el-table-column>
    ...
    <el-dialog :visible.sync="dialogFormVisible" @close="formData={}" width="50%">
      ...
      <el-form :mode="formData" label-width="120px">
        ...
        <el-form-item label="权限类型：">
          <el-input v-model="formData.type" autocomplete="off" clearable></el-input>
        </el-form-item>
        <el-form-item label="所属权限：">
          <el-cascader placeholder="选择权限"
...
export default {
  ...
  methods: {
    ...
    loadPrivilegeWithChild() {
      let url = "http://121.199.65.223:8888/privilege/findAllWithChild";
      get(url).then(res => {
        if (res.status == 200) {
          // 为页面数据赋值
    ...
    // 加载所有级联栏目
    loadPrivilege() {
      let url = "http://121.199.65.223:8888/privilege/findAll";
    ...
    // 提交表单
    submit() {
      let url = "http://121.199.65.223:8888/privilege/saveOrUpdate";
      // 获取表单数据
    ...
    del(pid) {
      this.$confirm("此操作将永久删除该数据, 是否继续?", "提示", {
        ...
        // 删除
        let url = "http://121.199.65.223:8888/privilege/deleteById";
        let data = { id: pid };
